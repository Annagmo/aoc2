...
int main( int argc, char *argv[ ] )
{
	...
	char *arquivoEntrada = argv[6];
		...
		int n_bits_offset =0, n_bits_indice =0, contador = assoc;
		...
		while (!feof(fptr))
		{
			...
			if (nsets == 1) {		
                ...
                for (i=0; i<assoc; i++)	{
                    if (cache_val[i] == 0) {					// Compulsory miss
					miss++; 
					miss_compulsorio++;
					cache_val[i] = 1;					// Coloca os valores no local
					cache_tag[i] = tag;
                    contador--;
				    } else { 
					    if (cache_tag[i] == tag) {                         // Hit
						    hit++;
					    } else { if (contador == 0)						// Capacity miss
						    miss++; 
						    miss_capacidade++;																 
			...
			if (nsets != 1 && assoc > 1) { //Conjunto Associativo
				for(i=0; i<assoc; i++){
					if(cache[indice][i].info == endereco &&  cache[indice][i].tag == tag && cache[indice][i].val == 1 ){ //i representa o conjunto naquela posiçao. Ex.: Assoc = 2 temos cache[indice][0] e cache[indice][1]
						hit++;
						CA_val_assoc[i] = 1; // não entendo porque no hit precisaria incrementar ou decrementar o contador se não muda nada na capacidade, fica igual
					}
						...
						CA_val_assoc[i] = 1; //contador--; // o compulsório ocupa uma pos vazia, ou seja, enche aquele lugar e preenche a capacidade
					} 
                    else if(cache[indice][i].val == 1 && acha0(CA_val_assoc,assoc) == true){ // conatdor != 0
						miss++;
                    	miss_conflito++;

                        if(*subst == 'r' || *subst == 'R'){ // já que só tem R
                            i = substitui(assoc);
                            cache[indice][i].tag = tag;
						    cache[indice][i].val = 1;
						    cache[indice][i].info = endereco;
						    CA_val_assoc[i] = 1;  // o conflito não preenche lugar disponível, só troca um valor por outro, acho que também não precisaria mexer no contador?
	                    }
					}                                             // daria pra deixar um else sem condição, se testar as duas em cima, acho
					else if(acha0(CA_val_assoc,assoc) == false){  // if (contador == 0)?
						miss++;
						miss_capacidade++;
						cache[indice][i].tag = tag;
						cache[indice][i].val =1;
...
int substitui(int assoc) {  // achei melhor deixar o teste na main, se tivessem outras formas
	srand(time(NULL));      // verificaríaos e enviaríamos para a função correspondente
	return (rand() % assoc-1);		
}
bool acha0(int *vetor,int tam){  // trocar tudo isso por um contador
	int i;
    for(i=0; i<tam; i++)
    {
        if(vetor[i]==0)
